...
def build_nn(pool_size, input_shape):
...
train_images = pickle.load(open("train_182_1.p", "rb" ))

# Load image labels
labels = pickle.load(open("labels_182_1.p", "rb" ))
...
num_rounds = 3
list_size = len(train_images) // num_rounds

train_list = [train_images[0:list_size], train_images[list_size:2*list_size], train_images[2*list_size:-1]]
label_list = [labels[0:list_size], labels[list_size:2*list_size], labels[2*list_size:-1]]

# plt.imshow(labels[0][:,:,0], interpolation='nearest')
# plt.show()
...
model = build_nn(pool_size, input_shape)
model.compile(optimizer='Adam', loss='mean_squared_error')

# split 17k training images into 3 pieces, and fit 3 times separately.
for i in range(num_rounds):
    train_images = train_list[i]
    labels = label_list[i]
...
model.save('CULane_CNN_model.h5')
